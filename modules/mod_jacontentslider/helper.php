...
class modJacontentsliderHelper
{
    ...
    function getListContent($catid, $params)
    {
		...
		$model->setState(
			'list.select',
			'a.id, a.title, a.alias, a.title_alias,a.images, a.introtext, ' .
			'a.checked_out, a.checked_out_time, ' .
			'a.catid, a.created, a.created_by, a.created_by_alias, ' .
			// use created if modified is 0
			'CASE WHEN a.modified = 0 THEN a.created ELSE a.modified END as modified, ' .
    ...
    function processIntrotext(&$row, $maxchars)
    {
		...
		if (function_exists ( 'mb_substr' )) {
			...
			$row->introtext =  SmartTrim::mb_trim ( ($row->introtext), 0, $maxchars, $doc->_charset );
			$row->introtext =  stripslashes($row->introtext);
			$row->introtext1 =  stripslashes($row->introtext);
		} else {
			$row->introtext = SmartTrim::trim ( ($row->introtext), 0, $maxchars );
			$row->introtext = stripslashes($row->introtext);
			$row->introtext1 = stripslashes($row->introtext);
		}
		return $row;
